...
- (void)makeTags {
    ...
    for (NSString *title in _myChannels) {
        LTChannelModel *model = [[LTChannelModel alloc] init];
        model.title = title;
        model.selected = NO;
        if (index == 0 || index == 1) {
            model.resident = YES;
        }
        model.channelType = LTChannelTypeMy;
    ...
    for (NSString *title in _recommendChannels) {
        ...
        model.title = title;
        model.selected = NO;
        model.channelType = LTChannelTypeRecommend;
...
- (void)setupUI {
    ...
    _dragingItem = [[LTChannelCell alloc] initWithFrame:CGRectMake(0, 0, UIDeviceScreenWidth/4-10, 53)];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    LTChannelCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:CellIdentifier forIndexPath:indexPath];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return CGSizeMake(UIDeviceScreenWidth/4-10, 53);
}
...
- (BOOL)collectionView:(UICollectionView *)collectionView canMoveItemAtIndexPath:(NSIndexPath *)indexPath{
    LTChannelCell *cell = (LTChannelCell *)[collectionView cellForItemAtIndexPath:indexPath];
    if (indexPath.section == 0) {
        if (cell.model.resident) {
            return NO;
        }else{
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        if (_isEdit) {  //编辑状态下
            //常驻的不可删除
            LTChannelModel *model = _myTags[indexPath.item];
        ...
        LTChannelModel *model = _recommendTags[indexPath.item];
        //设置cell的model
        model.channelType = LTChannelTypeMy;
        cell.model = model;
...
- (void)editTags:(UIButton *)sender{
    ...
    if (!_isEdit) {
        for (LTChannelCell *cell in _mainView.visibleCells) {
            if (cell.model.channelType == LTChannelTypeMy) {
                if (cell.model.resident) {
                    cell.delBtn.hidden = YES;
                }else{
                    cell.delBtn.hidden = NO;
...
- (void)dragBegin:(CGPoint)point {
    ...
    LTChannelCell *item = (LTChannelCell *)[_mainView cellForItemAtIndexPath:_dragingIndexPath];
    item.hidden = YES;
    //更新被拖拽的item
...
-(NSIndexPath *)getDragingIndexPathWithPoint:(CGPoint)point{
    ...
    LTChannelModel *model = _myTags[dragIndexPath.item];
    if (model.resident) {
        return nil;
    }
...
-(NSIndexPath*)getTargetIndexPathWithPoint:(CGPoint)point{
    ...
    LTChannelModel *model = _myTags[targetIndexPath.item];
    if (model.resident) {
        return nil;
    }
...
- (UICollectionView *)mainView {
    if (!_mainView) {
        ...
        [collectionView registerClass:[LTChannelCell class] forCellWithReuseIdentifier:@"cellIdentifier"];
