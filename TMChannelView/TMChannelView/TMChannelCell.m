...
@interface TMChannelCell ()

@end

@implementation TMChannelCell
...
- (void)setModel:(TMChannelModel *)model {
    _model = model;
    
    if (model.channelType == TMChannelTypeMy) {
        //如果标题包含“+” 则删除
    ...
    } else if (model.channelType == TMChannelTypeRecommend) {
        //如果标题不包含“+” 则添加
        if (![model.title containsString:@"+"]) {
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(5, 5, self.width - 10, self.height - 10)];
...
- (UIImageView *)delBtn {
    if (!_delBtn) {
        UIImageView *view = [[UIImageView alloc] initWithFrame:CGRectMake(self.width - 18, 0, 18, 18)];
