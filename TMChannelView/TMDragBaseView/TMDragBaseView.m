...
#import "TMDragBaseView.h"
#import "UIView+TMFrame.h"
#import "TMChannelDefine.h"

@interface TMDragBaseView ()<UIGestureRecognizerDelegate,CAAnimationDelegate>
...
- (void)layoutSubviews {
    ...
    self.dragContentView.frame = CGRectMake(0, _topSpace, self.width, self.height - _topSpace);
}
...
- (void)initialize {
    ...
    self.dragContentView.frame = CGRectMake(0, _topSpace, self.width, self.height - _topSpace);
    self.dragViewBg.frame = self.bounds;
...
- (void)panRecognizer:(UIPanGestureRecognizer *)gesture {
    ...
    if (state == UIGestureRecognizerStateChanged) { //移动
        CGFloat y = self.dragContentView.y;
        ...
        //背景的alpha
        self.dragContentView.centerY += point.y;
        CGFloat alpha = (1.0 - (y - self.topSpace) / self.dragContentView.height);
        ...
        CGFloat y = self.dragContentView.y;
        CGFloat maxOffset = self.dragContentView.height / 6;
